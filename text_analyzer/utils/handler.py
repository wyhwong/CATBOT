    ...
    def analyze(self, target_scores: dict) -> None:
        ...
        for target in targets:
            if target_news_prompts.get(target, None):
                target_scores[target]["news"] = self.text_inference.get_prompts_scores(target_news_prompts[target])
            if self.reddit_scraper and target_reddit_prompts.get(target, None):
                target_scores[target]["reddit"] = self.text_inference.get_prompts_scores(target_reddit_prompts[target])
            if self.twitter_scraper and target_twitter_prompts.get(target, None):
                target_scores[target]["twitter"] = self.text_inference.get_prompts_scores(
    ...
    def keywords_analysis(self, command_args: dict):
        ...
        if self.reddit_scraper:
            ...
            for prompt in reddit_prompts:
                results.append([self.text_inference.get_score(prompt), "reddit", prompt])
