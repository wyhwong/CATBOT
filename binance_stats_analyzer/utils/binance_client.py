...
LOGGER = get_logger(logger_name="BinanceAPI")
...
class BinanceClient:
    ...
    def query(self, targets: list) -> tuple:
        ...
        for target in targets:
            kline_dataframe = self.get_klines(symbol=target, start_str=start_str, end_str=end_str, interval=self.interval)
            num_trades_dataframe = self.get_number_of_trade(symbol=target, start_str=start_str, end_str=end_str, interval=self.interval)
