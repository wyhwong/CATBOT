...
class BinanceClient:
    ...
    def _get_historical_data(self, symbol: str, start_str: str, interval):
        dataframe = self.client.get_historical_klines(symbol=symbol, interval=interval, start_str=start_str)
        return pd.DataFrame(dataframe)

    def get_klines(self, symbol: str, start_str: str, interval: str) -> pd.DataFrame:
        dataframe = self._get_historical_data(symbol=symbol, start_str=start_str, interval=interval).loc[:, :5]
    ...
    def get_number_of_trade(self, symbol: str, start_str: str, interval) -> pd.DataFrame:
        dataframe = self._get_historical_data(symbol=symbol, start_str=start_str, interval=interval).loc[0:, (0, 8)]
    ...
    def query(self, targets: list) -> tuple:
        ...
        for target in targets:
            kline_dataframe = self.get_klines(symbol=target, start_str=start_str, interval=self.interval)
            num_trades_dataframe = self.get_number_of_trade(symbol=target, start_str=start_str, interval=self.interval)
